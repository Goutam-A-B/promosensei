...
export default function HomePage() {
  const [filters, setFilters] = useState<SearchParams>({ q: "", sort: "relevance", page: 1 });
    ...
    <main className="mx-auto flex min-h-screen max-w-6xl flex-col gap-6 px-4 py-8">
      <header className="flex flex-col gap-1">
        ...
        <p className="text-sm text-slate-500">
          Cross-platform offer discovery — Phase 1 (keyword search, Amazon).
        </p>
      </header>
      ...
      {!loading && data && (
        <>
          ...
          <ResultsGrid products={data.results} />
        </>
      )}
    </main>
  );
