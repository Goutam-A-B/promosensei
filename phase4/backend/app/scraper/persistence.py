...
def refresh_prices(
    ...
    *,
    ...
    Touches only price / discount / rating / last_seen_at on listings we
    already know about. The matcher is **not** run, no `Product` rows are
    created, and unknown `platform_product_id`s are counted as `skipped`
    rather than triggering a clustering pass. That's the whole point of
    the refresher — it's the cheap, high-frequency cousin of
    `upsert_listings`.

    `max_age_hours`, when set, restricts updates to listings staler than
    that. Listings refreshed by the live deals scrape within the window
    are left alone.
    """
    started = datetime.now(timezone.utc)
    result = ScrapeResult(platform=platform, started_at=started)
    ...
    if max_age_hours is not None:
        cutoff = datetime.now(timezone.utc) - _hours(max_age_hours)
    ...
    for scraped in listings:
        ...
        existing = existing_listings.get(scraped.platform_product_id)
        if existing is None:
            # New listing → out of scope for the refresher; the full scrape
            # job is responsible for creating it.
            result.skipped += 1
            continue
        # SQLite drops tz info on read — normalise both sides before comparing.
        if cutoff is not None and existing.last_seen_at is not None:
        ...
        try:
            ...
            existing.discount = scraped.discount
            if scraped.rating is not None:
                existing.rating = scraped.rating
            existing.last_seen_at = datetime.now(timezone.utc)
            result.updated += 1
        except Exception as exc:  # noqa: BLE001
            logger.exception("Refresh failed for %s/%s: %s", platform, scraped.platform_product_id, exc)
            result.errors += 1

    if result.errors > 0 and result.updated == 0:
        run.status = "failed"
    elif result.errors > 0:
        run.status = "partial"
    run.updated = result.updated
    ...
    result.finished_at = run.finished_at
    logger.info(
        "Refresh %s: updated=%d skipped=%d errors=%d (status=%s)",
        platform, result.updated, result.skipped, result.errors, run.status,
    )
    return result
